...
**8k の 9B は、この6ケースでは Claude Haiku 4.5 を上回った。**
正答 18/18 に対して Haiku は 15/18、指示追従も 18/18 対 17/18。
秒数は3倍かかるが、0円で、手元から出ない。

ただし**6ケース×3周でしかない。** 「ローカルが Haiku を超えた」ではなく
「この6問では差が出なかった」が正しい読み方。
Haiku の3件の失点は下に書いた「効きすぎた注記」で、性能というより
道具側の設計に起因する。

4k の失点3件はすべて `amagasaki-basic` の `truncated/length`。
**同じモデル・同じ重みで、設定だけで 83% と 100% に割れる。**
`num_ctx` は速度の設定に見えて、正答率の設定でもあった。

**正答率は同じだが中身が違う。** 列Aは道具の説明文に埋めた全国値を拾って
偶然当てていて、指示された基準値の確認をしていない。列Dは毎回引く。
道具が増えるほどこの差が効く。

尼崎市に `pref='兵庫県'` を正しく渡すのも列Dだけだった。列Bの9Bは `'大阪'` を
渡して該当0にし、回復に2,946トークン使ってコンテキストを使い切っている。
**賢いモデルは回復が上手いのではなく、回復が要る状況を作らない。**
