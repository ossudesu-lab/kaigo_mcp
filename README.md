...
| B | qwen3.5:9b | RTX 5050 8GB | 0円 | 計測済み（`num_ctx` 4k / 8k の2本） |
| C | Qwen3.5-397B-A17B | DeepInfra | 約0.8円 | **保留**（登録にカードが要る可能性） |
| D | Claude Haiku 4.5 | Anthropic | 約1.4円（実測） | 計測済み |

**BとCを同じ Qwen3.5 系で揃えている。** そうすると B→C の差が「モデルサイズだけの効果」、
...
この6ケースでは 8k の 9B が上回っている（正答 18/18 対 15/18、
...
- 列Bのどちらを正式な列にするか未決（4k は列Aと条件が揃う／8k は答えが出る）
